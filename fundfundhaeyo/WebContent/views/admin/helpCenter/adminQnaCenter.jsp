	    	...
	    	<div class="adm-content-center-main">
	    		...
	    		<div class="adm-content-center-main-body">
	    			...
	    			<div class="adm-content-center-main-body2">
	    				...
	    				<% } %>
	    					...
	    					<tr>
	    						...
	    						<td class="center"><%= list.get(i).getQnaNo() %></td>
	    						<td><%= list.get(i).getQnaTitle() %></td>
	    						<td><%= list.get(i).getQnaContent() %></td>
	    						<td class="center"><%= list.get(i).getQnaCreateDate() %></td>
	    						<% if(list.get(i).getQnaAnswerStatus().equals("Y")){ %>
	    						<td class="center">답변완료</td>
	    						<% }else{ %>
	    						<td class="center">답변대기</td>
	    						<% } %>
	    						<% if(list.get(i).getQnaStatus().equals("Y")){ %>
	    						<td class="center">게시</td>
	    						<% }else{ %>
	    						<td class="center">비게시</td>
	    						<% } %>
	    						<td class="center"><a href="admMoveToUpdateQnaForm.hp?qNo=<%= list.get(i).getQnaNo() %>"><i class="hoverA2 fa-regular fa-pen-to-square fa-lg"></i></a></td>
	    					</tr>
	    					<% } %>
	    				</tbody>
	    			</table>
	    			...
	    			<div class="adm-content-center-main-body3">
	    				...
	    				<% if(currentPage != 1){ %>
		            		<button type="button" onclick="location.href='admQna.hp?cpage=<%= currentPage-1 %>'">←</button>
			        	<% }else{ %>
			        		<button type="button" onclick="location.href='admQna.hp?cpage=<%= currentPage-1 %>'" disabled>←</button>
			        	<% } %>
			            ...
			            <% for(int p = startPage; p <= endPage; p++){ %>
			            	...
			            	<% }else{ %>
			            		<button type="button" onclick="location.href='admQna.hp?cpage=<%= p %>'"><%= p %></button>
			            	<% } %>
			            <% } %>
