...
/**
 * Servlet implementation class QNACenterController
 */
@WebServlet("/qna.hp")
public class QNACenterController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public QNACenterController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<QNA> list = new QNAService().selectQNALimitList(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		
		request.getRequestDispatcher("views/helpCenter/qnaCenter.jsp").forward(request, response);
	}
